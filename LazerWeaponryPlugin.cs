...
namespace LazerWeaponry
{
    ...
    [BepInPlugin(MyPluginInfoNew.PLUGIN_GUID, MyPluginInfoNew.PLUGIN_NAME, MyPluginInfoNew.PLUGIN_VERSION), ContentWarningPlugin(MyPluginInfoNew.PLUGIN_GUID, MyPluginInfoNew.PLUGIN_VERSION, false)]
    ...
    public class LazerWeaponryPlugin : BaseUnityPlugin
    {
        ...
        internal const uint MYCELIUM_ID = 3913099;
        ...
        private void Awake()
        {
            ...
            Logger.LogInfo($"{MyPluginInfoNew.PLUGIN_NAME} by {MyPluginInfoNew.PLUGIN_GUID.Split(".")[0]} has loaded!");
        }
        ...
        {
            SyncedSettings.sync_PVPMode.ConfigChanged += delegate { Logger.LogWarning($"Host dude changed PvP mode to {InitialSettings.PVPMode}!"); };
            SyncedSettings.sync_Damage.ConfigChanged += delegate { Logger.LogWarning($"Host dude changed damage to {InitialSettings.Damage}!"); };
            SyncedSettings.sync_MaxAmmo.ConfigChanged += delegate { Logger.LogWarning($"Host dude changed max ammo to {InitialSettings.MaxAmmo}!"); };
            SyncedSettings.sync_DelayAfterFire.ConfigChanged += delegate { Logger.LogWarning($"Host dude changed delay after fire to {InitialSettings.DelayAfterFire}!"); };
            SyncedSettings.sync_HeadshotDamageMultiplier.ConfigChanged += delegate { Logger.LogWarning($"Host dude changed headshot damage multiplier to {InitialSettings.HeadshotDamageMultiplier}!"); };
            SyncedSettings.sync_MonsterFallTime.ConfigChanged += delegate { Logger.LogWarning($"Host dude changed monster fall time to {InitialSettings.MonsterFallTime}!"); };
            SyncedSettings.sync_MonsterHitForceMultiplier.ConfigChanged += delegate { Logger.LogWarning($"Host dude changed monster force multiplier on hit to {InitialSettings.MonsterHitForceMultiplier}!"); };
            SyncedSettings.sync_RecoilForce.ConfigChanged += delegate { Logger.LogWarning($"Host dude changed recoil force to {InitialSettings.RecoilForce}!"); };
            SyncedSettings.sync_PlayerKillReward.ConfigChanged += delegate { Logger.LogWarning($"Host dude changed player kill reward to {InitialSettings.PlayerKillReward}!"); };
            SyncedSettings.sync_MonsterKillReward.ConfigChanged += delegate { Logger.LogWarning($"Host dude changed monster kill reward to {InitialSettings.MonsterKillReward}!"); };
            SyncedSettings.sync_VulnerableEnemies.ConfigChanged += delegate { Logger.LogWarning($"Host dude changed vulnerable enemies to {InitialSettings.VulnerableEnemies}!"); };
            SyncedSettings.sync_HeadshotFallTime.ConfigChanged += delegate { Logger.LogWarning($"Host dudeseter changed headshot fall time to {InitialSettings.HeadshotFallTime}!"); };
        }
            ...
            _spawnedProjectile.hitAction = (Action<RaycastHit>)Delegate.Combine(_spawnedProjectile.hitAction, delegate (RaycastHit hit)
            {
                ...
                if (_hitPlayer != null)
                {
                    ...
                    if (_hitPlayer.data.sinceDied == 1000 && _hitPlayer.data.dead)
                    {
                        ...
                        Transform topmostParent = hit.collider.transform;
                        while (topmostParent.parent != null && topmostParent.parent.gameObject != GameObject.Find("Game"))
                        {
                            topmostParent = topmostParent.parent;
                        }
                        UnityEngine.Object.Destroy(topmostParent.gameObject, 15);
                        Logger.LogWarning($"HIT: {hit.collider.gameObject.name}");
                        Transform currentParent = hit.collider.transform.parent;
                        while (currentParent != null)
                        {
                            Logger.LogWarning($"PARENT: {currentParent.gameObject.name}");
                            currentParent = currentParent.parent;
                        }
                        Logger.LogWarning($"DESTROYING {(topmostParent.gameObject.name)} In 15 Seconds!");
